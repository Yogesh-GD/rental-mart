...
export const getFeaturedVehicles = createAsyncThunk("/vehicles/featured", async (query,{ rejectWithValue }) => {
    try {
        const response = await api.get(`/api/v1/get/vehicles?${query || ""}`)
        return response.data.data
    } catch (error) {
        return rejectWithValue(error.data)
    }
...
export const getVehiclesByAdmin = createAsyncThunk("/vehicles/get/admin", async (query,{ rejectWithValue }) => {
    try {
        const queryString = new URLSearchParams(query).toString();
        const response = await api.get(`/admin/get/vehicles?${queryString}`);
...
export const getVehicleDetailsByAdmin = createAsyncThunk("/vehicle/details/get/admin", async (id,{ rejectWithValue }) => {
    try {
        const response = await api.get(`/admin/get/vehicle/details/${id}`)
        return response.data.data
    } catch (error) {
        return rejectWithValue(error.data)
    }
...
const vehicleSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getFeaturedVehicles.pending,( state ) => {
                state.error = null,
                state.loading = true
            })
            .addCase(getFeaturedVehicles.fulfilled, ( state, action ) => {
                state.featured = action.payload.data.vehicles
                state.pagination = action.payload.pagination
                state.loading = false
                
            })
            .addCase(getFeaturedVehicles.rejected, ( state, action ) => {
            ...
            .addCase(getVehiclesByAdmin.pending,( state ) => {
                state.error = null,
                state.loading = true
            })
            .addCase(getVehiclesByAdmin.fulfilled, ( state, action ) => {
                state.vehicles = action.payload.data.vehicles
                state.pagination = action.payload.pagination
                state.loading = false
                
            })
            .addCase(getVehiclesByAdmin.rejected, ( state, action ) => {

            })
            .addCase(getVehicleDetailsByAdmin.pending,( state ) => {
                state.error = null,
                state.loading = true
            })
            .addCase(getVehicleDetailsByAdmin.fulfilled, ( state, action ) => {
                state.details = action.payload
                state.loading = false
                
            })
            .addCase(getVehicleDetailsByAdmin.rejected, ( state, action ) => {
